...
> **Nível ROM, sobre premissas.** Este documento desenha a arquitetura da solução em nível de proposta, sobre seis premissas de arquitetura já formalizadas — residência de dados híbrida (ADR 0001), **instância Salesforce dedicada e 100% greenfield (ADR 0002)**, **fronteira CRM-não-transacional / motor de regras de split (ADR 0003)**, **plataforma sobre Sales Cloud reusando objetos nativos Opportunity/Quote (ADR 0004)**, **soberania de dados via de-tokenização on-premise (ADR 0005, parcialmente superseded)** e **MuleSoft reusando a instalação on-premise já existente da Dataprev (ADR 0006)** — e três decisões ainda em aberto (fronteira campo-a-campo da residência, identidade Experience Cloud × CPF, contratos de API), além da seleção do provedor do gateway. Essas decisões, o provisionamento da org greenfield, a contratação do gateway e os acessos/ambientes/capacidade na instalação MuleSoft on-premise existente se resolvem **dentro da Fundação (Semanas 1-4)** — não há mais uma Etapa 0 isolada: a definição de arquitetura foi mesclada ao Planning & Design. As premissas ADR 0004/0006 foram **assumidas pela equipe em 31/jul–03/ago/2026 e ainda a validar com o cliente** — carregam etiqueta Assumed. Cada decisão load-bearing carrega sua etiqueta de origem — `[KB: …]`, `[extends: …]` ou `[assumption: …]`.
...
Os quatro domínios do programa (marketplace de cotação, credenciamento, financeiro/split e atendimento) vivem numa **única org**, mas essa org é **dedicada e apartada** das demais orgs de clientes da Dataprev. O isolamento não é preferência — é **forçado** por segurança e sensibilidade financeira, pela volumetria (~800 mil estabelecimentos, ~450 mil beneficiárias — a ratificar), pela auditabilidade perante TCU/CGU/ANPD e pela administração da instância pelo próprio MTE (que não pode enxergar "a cozinha" dos demais clientes). O provisionamento tem lead-time externo e é **um dos dois destravamentos externos resolvidos dentro da Fundação (Semanas 1-4)** — não há mais Etapa 0 isolada. Esta premissa permanece **inalterada** pelo reuso do MuleSoft (ADR 0006): a org Salesforce continua 100% greenfield. A separação entre domínios internos segue por objeto, perfil e modelo de compartilhamento. `[decisions/0002-instancia-dedicada-mte-pat]` — premissa ainda **verbal, a ratificar por escrito**.

### Plataforma sobre Sales Cloud, reusando objetos nativos (ADR 0004)

A plataforma é construída sobre **Sales Cloud**, mapeando o leilão reverso em **objetos nativos** em vez de objetos 100% custom, em prol do curto tempo de implantação sob a data fixa. **Opportunity (nativo)** = a demanda que a beneficiária cadastra (nº de trabalhadores, valor, vigência, distribuição por UF, recursos obrigatórios) — reusa estágios, ownership, activity history e relatórios nativos. **Quote (nativo)** = a resposta da facilitadora ao leilão reverso, recebida **via API** (MuleSoft), uma por facilitadora, associada à Opportunity. A facilitadora é **API-only** (sem tela, sem assento de portal): descobre as demandas abertas na vigência consultando um **endpoint de consulta (pull via API)** na Fase 1 — **não há push ativo** (a notificação ativa é roadmap futuro, com o canal a definir — o que decide se posicionamos ou não Marketing Cloud). A **equidade é por construção**: como a facilitadora não tem tela, não enxerga a proposta concorrente — **sem Apex managed sharing**. A camada custom fica reservada ao que os nativos não entregam: a tela "Comparar Propostas" (LWC) para a beneficiária, a **trava de seleção até o fechamento da janela de vigência** (a beneficiária vê as cotações conforme chegam, mas só seleciona quando a janela fecha — não é seleção cega) e a transição seleção→contrato. `[decisions/0004-sales-cloud-objetos-nativos]` — premissa assumida em 31/jul, **a validar com o cliente** (Assumed). A complexidade migra de "construir do zero" para "adaptar o nativo sell-side ao padrão comprador→N-vendedores".

### Org Salesforce greenfield + MuleSoft reusa o on-premise existente — soberania e isolamento (ADR 0002 + ADR 0006, supersede parcial da ADR 0005)

A **instância Salesforce dedicada segue 100% greenfield**: provisionada do zero, sem reaproveitar nenhuma org, metadado ou administração de outros ambientes Dataprev — isolamento por construção (ADR 0002, inalterada). O **MuleSoft**, por sua vez, **não é mais greenfield**: a Dataprev **já opera uma instalação MuleSoft on-premise em produção**, com infraestrutura, observabilidade e modelo operacional em pé — o programa **reusa essa instalação existente** em vez de provisionar uma nova (ADR 0006, confirmado com a arquitetura Dataprev em 03/ago/2026, supersede parcialmente a ADR 0005). É o ponto de de-tokenização do CPF (ADR 0001) e o trilho de soberania — o dado sensível não sai do perímetro soberano. O reuso **elimina o lead-time de aprovisionamento do MuleSoft** (as antigas 2 semanas de Etapa 0) e permite **mesclar a definição de arquitetura ao Planning & Design da Fundação**, que continua com 4 semanas mas agora **inicia na Semana 1**. Resta a dependência de **acessos, ambientes e capacidade** na instalação existente (Client IT/Plataforma) — não mais a instalação em si. Reuso da plataforma não é reuso das APIs: a camada API-led ainda é desenhada e construída net-new, como uma **frente MuleSoft contínua (E05)** por todo o Planning & Design e desenvolvimento (S1-S11), sob um **Arquiteto Técnico MuleSoft** dedicado. `[decisions/0006-mulesoft-reuso-onpremise]` `[decisions/0005-greenfield-mulesoft-onpremise]` — **G0504 permanece resolvido como on-premise** (muda de "instalar" para "reusar"), premissa assumida em 31/jul–03/ago, a validar (Assumed).
...
| MuleSoft Anypoint — **on-premise (reuso, ADR 0006)** | Camada de integração API-led para todos os sistemas externos, o **gateway** e as ~600-700 facilitadoras; roda sobre a instalação on-premise **já existente** da Dataprev — frente contínua (E05, S1-S11) liderada por um Arquiteto Técnico MuleSoft dedicado |
| Agentforce | Atendimento inteligente (E06 — **fora da Fase 1**, candidato a de-escopo) |
...
**Três decisões de arquitetura em aberto que esta fundação carrega até serem resolvidas na Fundação (Semanas 1-4):**
...
**Abordagem de solução.** Camada de integração API-led (System / Process / Experience) rodando sobre a instalação **on-premise já existente da Dataprev** (ADR 0006, reuso — supersede parcialmente a ADR 0005; não CloudHub; é o ponto de de-tokenização do CPF e o trilho de soberania). Reuso da plataforma não é reuso das APIs: os contratos, o roteamento e a integração do gateway são net-new, entregues por uma **frente MuleSoft contínua** que corre por todo o Planning & Design e desenvolvimento (S1-S11), sob um **Arquiteto Técnico MuleSoft** dedicado (integral nas 4 primeiras semanas, 20h/sem nas semanas de dev). Abordagem **mock-first**, obrigatória porque o **Novo PAT não tem API hoje** e nenhum Swagger/contrato dos sistemas externos foi disponibilizado. O contrato de integração com o gateway (ADR 0003) e a origem do termo de aceite (INIS PJ / Kinis PJ) são alvos declarados já na Fundação. A conciliação financeira chega por **lotes incrementais via agendamento MuleSoft**; a consulta à adquirente + monitoramento alimentam E04. Além da recepção das Quotes e da devolução de processado+valor, a camada **expõe às facilitadoras um endpoint de consulta das demandas/leilões abertos na vigência** — pull na Fase 1 (a facilitadora consulta, sem push ativo); a **notificação ativa/push é roadmap futuro, com o canal a definir** (o que decide se posicionamos ou não Marketing Cloud — G0209/G0211).

**Arquitetura de suporte.** Padrões event-driven onde couber (Platform Events/CDC), tratamento de erro/replay, e governança de virada dos mocks para APIs reais. `[decisions/0006-mulesoft-reuso-onpremise]` `[decisions/0005-greenfield-mulesoft-onpremise]` `[KB: salesforce-revenue-cloud-marketplace-arch.md:36-40]` `[assumption: G0501 — ausência total de contratos de API (Novo PAT sem API) é o risco #1 e caminho crítico; G0309 — integração do gateway]` *(G0504 permanece resolvido — MuleSoft on-premise, agora reuso via ADR 0006.)*
...
**Abordagem de solução.** Materializa a fundação de residência sobre **três eixos de isolamento/soberania**: (1) **tokenização** — CPF e sensíveis não persistem no Salesforce (ADR 0001); (2) **instância Salesforce dedicada, apartada e 100% greenfield** — nenhuma org/metadado/administração compartilhada, nenhum admin de outro ambiente enxerga estes dados (ADR 0002); (3) **MuleSoft on-premise reusando a instalação já existente da Dataprev** — o trilho de integração roda dentro do perímetro soberano e a de-tokenização ocorre on-premise (ADR 0006, supersede parcialmente a ADR 0005; o reuso é exclusivo do MuleSoft, não muda o isolamento da org Salesforce). Acompanham diagrama de fluxo de dados sob LGPD e trilha de auditoria para acesso a dados sensíveis que não estão na org.

**Arquitetura de suporte.** Tokenização + resolução runtime via MuleSoft on-premise reusado; isolamento da org greenfield dedicada; overlay de conformidade regulada. `[decisions/0006-mulesoft-reuso-onpremise]` `[decisions/0005-greenfield-mulesoft-onpremise]` `[assumption: G0801 — fronteira exata de campos a ratificar com arquitetura Dataprev; é a decisão que governa o data model de várias épicas]`
...
**Abordagem de solução.** Plano de comunicação, materiais de apoio, capacitação e métricas de adoção. Entrega Salesforce PS. Sem arquitetura técnica bespoke. Na Fase 1, adoção enxuta; programa completo pós-go-live (buffer).
...
O programa opera em **modo data-fixa**, planejado de trás para frente a partir de datas comprometidas: **início 17/ago/2026 → homologação a partir da entrega do Marketplace → go-live PRODUÇÃO 15/nov/2026 → Scale/Hypercare 16/nov–13/dez/2026** (interoperabilidade total do Decreto 12.712/2025 + entrada do financeiro em produção). A data é imóvel, e o escopo é a variável de flexão. A duração total do programa é de **17 semanas — 13 de build (Fase 1) + 4 de Scale/Hypercare**. A estratégia de execução é **definir o modelo de dados fundacional (objetos nativos Opportunity/Quote + termo de aceite) com o time inteiro primeiro, já na Fundação (Semanas 1-4)** e, sobre essa base, **paralelizar as frentes de baixa dependência** — é o que torna a data viável. Com o **reuso do MuleSoft on-premise (ADR 0006)**, a antiga Etapa 0 de aprovisionamento (2 semanas) foi eliminada e a definição de arquitetura mesclada ao Planning & Design da Fundação, que agora inicia na Semana 1; a folga liberada foi redistribuída em janelas de desenvolvimento mais longas e paralelas (Marketplace S5-S10, Financeiro S6-S11), reduzindo o risco de prazo. A Fase 1 entrega oito das nove épicas (E06/Agentforce fica de fora), com a integração corporativa (E05) rodando como **frente MuleSoft contínua por todo o Planning & Design e desenvolvimento (S1-S11)**, liderada por um Arquiteto Técnico MuleSoft dedicado. Os quatro semanas finais de Scale/Hypercare (S14-S17) sustentam, mantêm e conduzem o cutover à Dataprev, sem novo desenvolvimento. O `roadmap` detalha as seis fases (com marcos de projeto, de decreto e de entrega de jornadas para UAT e PROD), o caminho crítico e os candidatos a de-escopo que servem de buffer.

> ⚠ **Viabilidade da data fixa — sinalizada.** 13 semanas de build para um escopo XL regulado (financeiro com split, conciliação e emissão de boleto) sobre **dois destravamentos externos** — provisionamento da org 100% greenfield e seleção/contratação do gateway — **mais acessos, ambientes e capacidade** na instalação MuleSoft on-premise já existente da Dataprev (o antigo pré-requisito "instalação on-premise pronta a tempo" caiu com o reuso, ADR 0006) — continua um cronograma **agressivo**, com margem mínima. Se qualquer um desses destravamentos atrasar ou a homologação achar defeito no financeiro, **15/nov não é alcançável só com esforço adicional**: o de-escopo (E03 financeiro primeiro, adiando parte do split/conciliação para pós-go-live) é o único trilho para preservar a data. A resolução dos três blockers de arquitetura restantes **mais** o provisionamento da org greenfield, a contratação do gateway e a confirmação de acessos ao MuleSoft existente, tudo já dentro da Fundação, são a recomendação que antecede tudo.
