...
Relevance: AI-native delivery methodology for Agentforce/Data Cloud programs. Use to inform phase structure, effort distribution, team operating model, and customer fit criteria.
...
## AI-Native vs Traditional Delivery
Timeline Example: Traditional 32 weeks → AI-Native 20 weeks
Phase cycle time reduction: 35-40%
Stakeholder engagement: Traditional Intermittent → AI-Native Continuous/Daily
Data readiness: Traditional delays tolerated → AI-Native Data ready Week 1
Build approach: Traditional manual config + some AI → AI-Native agentic build + human refinement
...
### Prepare
Team onboarding, project plan generation, deep research, discovery, delivery handoff.
AI acceleration: automated metadata discovery (MeshMesh), rapid prototyping (Cursor), smart documentation.

### Design (Intent)
Generate process maps, create user stories, review solution design & UX.
Generate data model & strategy, rapid prototyping, environment strategy, integration strategy.
Intent is understood and articulated on Day 1 — we arrive with a perspective mined from discovery calls.

### Deliver (Build & Refine)
User story refinement, generate code/config, generate mock data.
QA plan and test cases, release plan, DevOps, security posture.
Daily Demo = quality gate AND status report.

### Deploy
Solution documentation, enablement guides, org config mining, Human Centered Change.

### Govern
Meeting follow-up, monitoring.

## New Ways of Working
1. Micro-Incremental Validation: shift from bi-weekly sprint reviews to daily 15-minute async prototype interaction sessions. SMEs review agent outputs daily.
2. Human-in-the-Loop Collaboration: stakeholders stop signing off on static BRDs. They co-pilot prompts and immediately test generated logic.
3. Automated Bureaucracy Bypass: AI-driven compliance and governance checks auto-approve low-risk changes.

## Hybrid PS Squad — Roles

Customer Roles:
- Customer Product Owner — defines goals, success metrics, guardrails
- Customer SME — provides feedback on agent outputs
- Customer Data Steward — validates data hygiene and grounding
- Customer Security SME — reviews guardrails, hallucination checks

SF Pro Services Roles:
- Intent Architect — defines goals, success metrics, guardrails
- Agent Orchestrator — selects tools, focuses on guardrails, technical solution, data model
- Logic & Integration Engineer — builds specialized functions agents call
- Interaction and Trust Designer — Agentforce Specialist, creates prompts, conversational design
- Adoption Architect — designs HITL touchpoints, conversational design
- Logic Validator — ensures solutions deliver on design and intent

PS Agent Roles (Digital Twins):
- Digital Intent Twin — validates goals, success metrics, guardrails
- Digital Orchestrator Twin — validates solution and architecture design
- Digital Engineering Twin — builds on Salesforce platform using intent and guardrails
- Digital Adoption Twin — adoption validation
- Logic Validator Twin — quality and speed assurance
...
## New Delivery Mindset
- The New Day 1: Intent is understood and articulated on Day 1. We arrive with a perspective mined from discovery calls, integration landscapes, and business outcomes before kickoff.
- Outcomes are the Goal: Speed & Quality = Trust. The fastest path to a production-ready solution is also the highest-quality path.
- Feedback is the Product: Feedback is not a ceremony, it is everything. The daily demo is the quality gate and our status report.
